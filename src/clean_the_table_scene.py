...
objs = bproc.loader.load_blend("/home/sorin/data/blenderproc/data/saved_scenes/clean_up_table.blend")
...
def init_objects():
    ...
    mug = bproc.filter.one_by_attr(objs, "name", "MetalMug")
    bowl = bproc.filter.one_by_attr(objs, "name", "Bowl")

    list_of_objects = [knife, spoon, fork, plate, mug, bowl]
...
def camera_poses(x):
    for i in range(x):
        ...
        location = np.random.uniform([3.1, -2.6, 1.6], [3.5, -2.6, 1.8])
...
def deploy_scene(x):
    for i in range(x):
        ...
        random_objects = random.sample(lob, 3)
